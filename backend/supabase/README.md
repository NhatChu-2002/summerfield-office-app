...
The preparer explicitly excludes `202609160003_add_seasonal_drinks.sql`: it seeds real Summerfield product data and aborts on a clean database without the existing organization and vendor. On 2026-09-25, a separate unlinked project at `../summerfield-hq-replay-20260925` replayed all 46 current schema migrations from both repositories in filename order. Its ledger recorded 46 versions, from `202609090001` through `202609250004`, and both HQ pgTAP files passed (56 assertions). That project was stopped with its local database backed up; the original QA stack and its synthetic reports were not reset. This validates the clean schema/RLS replay, **not** the excluded production-data seed or the hosted migration ledger. CI still runs only offline checks because the inventory repository is private and its migration checkout is not available to the HQ workflow token; add a read-only cross-repository credential or another reproducible source before requiring pgTAP in CI.
...
The connected HQ Ticket desk uses inventory-owned `202609210001_tickets.sql` without a new migration. Its `create_ticket`, `list_tickets`, `get_ticket`, `update_ticket_status`, `assign_ticket`, and `list_ticket_events` RPCs remain the database permission boundary. HQ also uses its `list_hq_department_people` RPC to offer department assignees. `ticket_workflow.test.sql` exercises store-scoped submission, reporter/lead/admin visibility, reviewer-only status and assignment, revision conflicts, activity, and organization isolation against an isolated local database. The ticket migration was unrecorded in the last hosted ledger snapshot; do not enable this connected workflow on the hosted project until that ledger and the related HQ migration are reconciled and applied in reviewed order.
