...
Inventory `202609260004_store_inspection_lead_access.sql` keeps `store_inspections` and its existing photo/storage policies in the inventory-owned schema. It allows active Operations or Stores (`store_manager` department) leads to read and edit inspections at any active store in their organization; admins and assigned store managers keep their prior access. Other department leads do not gain inspection access. HQ uses `list_store_inspection_stores` for the store picker and `list_store_inspection_page` for a cursor-paged history, then the existing revision-checked draft RPC. No inventory rows are deleted or rewritten. The first HQ slice connects history plus store details, arrival, and order draft fields only; scoring, safety, photos, submission, and PDF are not yet ported. The local QA stack received this SQL directly in one transaction without a reset, preserving synthetic records but **not** repairing its historical ledger. The 50-version disposable replay is the verification evidence. Apply this inventory-owned migration to hosted Supabase only after ledger reconciliation, backup, and RLS review, before exposing the connected HQ page there.
