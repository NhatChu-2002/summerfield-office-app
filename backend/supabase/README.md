...
Docker Desktop and a local Supabase stack were verified on 2026-09-25. The QA CLI project is at `../summerfield-hq-local-supabase`, outside both Git repositories and not linked to the hosted project. It contains synthetic QA records; do not reset it for schema replay. To rebuild an empty stack on another machine, initialize a separate directory with `npx --yes supabase@2.118.0 init`, then from this HQ repository run:
...
| Inventory `202609260003_ticket_lead_routing.sql` | Inventory ticket and assignee-read migrations; clean 49-migration replay and 168 HQ pgTAP assertions passed | Pending reconciliation; **not** applied to hosted Supabase |
| Inventory `202609260004_store_inspection_lead_access.sql` | Inventory inspection schema and department memberships; clean 50-migration replay and 197 HQ pgTAP assertions passed | Pending reconciliation; **not** applied to hosted Supabase |
...
Inventory `202609260003_ticket_lead_routing.sql` extends the ticket contract: an active department lead may submit for any active organization store, read the organization-wide ticket queue, and reroute a ticket to another supported department with a revision check and activity record. The prior assignee is cleared on reroute. Status and teammate assignment stay limited to the destination department's lead or organization admin. A lead may optionally assign an active teammate while submitting to a department they lead; the create-and-assign RPC is atomic. Store managers remain limited to their assigned stores and own reported tickets. Ordinary department members do not gain ticket submission in this slice. HQ reads a bounded, cursor-paged queue through `list_ticket_page` and gets submission stores through `list_ticket_stores`. The migration is pending hosted-ledger reconciliation and has **not** been run against production inventory data.

The local QA stack received that inventory migration directly in one transaction after clean replay verification; its priority-queue index was added after the final migration revision. This preserved the synthetic QA records but did not add a version to the QA stack's historical migration ledger. Do not use that ledger as evidence for hosted deployment. The disposable 49-version replay is the migration-order evidence.

Inventory `202609260004_store_inspection_lead_access.sql` keeps `store_inspections` and its existing photo/storage policies in the inventory-owned schema. It allows active Operations or Stores (`store_manager` department) leads to read and edit inspections at any active store in their organization; admins and assigned store managers keep their prior access. Other department leads do not gain inspection access. HQ uses `list_store_inspection_stores` for the store picker and `list_store_inspection_page` for cursor-paged history. The connected form now reads and writes the legacy v1 inspection payload, including weighted scoring, food-safety gate, notes, follow-up, and conditional drive-thru; it uses the existing revision-checked draft/submit/reopen and photo RPCs. Manager and internal PDFs are generated in the frontend. No additional migration is required for this form slice, and no inventory rows were deleted or rewritten. The local QA stack received the lead-access SQL directly in one transaction without a reset, preserving synthetic records but **not** repairing its historical ledger. The 50-version disposable replay is the verification evidence. Apply this inventory-owned migration to hosted Supabase only after ledger reconciliation, backup, and RLS review, before exposing the connected HQ page there. Browser photo-file upload remains to be verified in an environment that permits the file chooser; pgTAP validates metadata access and submission locks, not a binary Storage transfer.
