...
The local pgTAP suite passes 42 contract checks with synthetic admins, leads, members, viewers, store managers, inactive accounts, and another organization. The separate report-history test passes eight checks for cross-period reads, status/pagination behavior, department isolation, and cross-organization isolation. The full versioned template payload still needs coverage, and pgTAP must run in CI before hosted deployment. The hosted project is not a fixture environment; this audit made no data changes there.
