...
This part covers how HQ is hosted, configured, and checked. Read `../AGENTS.md` for repository-wide rules.
...
- **Database migrations are not deployed by CI.** They are applied by hand, test project first (see `../backend/AGENTS.md`). Deploy a migration before the frontend or API that depends on it.
- **The existing inventory app's Render services are outside this repo.** Nothing here may change them.

## Unverified

These settings haven't been checked against a live Render account yet. Check them when connecting:

- The Blueprint file path (`infra/render.yaml`).
- Static-site header support.
- Environment-specific Render services, variables, and deployment readiness. The Git remote and CI workflows are connected.
