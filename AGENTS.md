...
This is the canonical guidance for any AI agent working in this repository. Read it first, then read the nearest scoped `AGENTS.md` for the part you change: `frontend/`, `backend/`, or `infra/`. Human setup and runbooks remain in the corresponding `README.md` files; design evidence remains under `docs/design/qa/`. The root `CLAUDE.md` only points here for Claude-based tools.
...
```
summerfield-hq/
├── AGENTS.md                 repository-wide guidance (you are here)
├── CLAUDE.md                 compatibility pointer to AGENTS.md
├── README.md                 short human overview and quick start
├── frontend/                 React + TypeScript + Vite app            → frontend/AGENTS.md
...
│   │                         (auth, dashboard, departments, tasks, updates, calendar, ownership, projects, watch, workspace, sunny)
...
├── infra/                    Render Blueprint, environments, deploy runbook → infra/AGENTS.md
...
1. **Pick one part and stay in it.** Before editing, read that part's `AGENTS.md`. Run that part's checks, not the whole repo's.
...
```
feat/*     ──PR──▶  dev  ──PR──▶  test  ──PR──▶  stage  ──PR──▶  main
...
| --- | --- | --- | --- |
| `feat/<short-name>` | One change, branched from fresh `dev`. Use `fix/<name>` for bugs and `chore/<name>` for tooling. | Feature contributor | Local checks pass before PR. |
| `dev` | Everyone's finished work, integrated | PR from a feature branch | Relevant CI green; required review if branch protection asks for it. |
| `test` | Release candidate for QA | PR from `dev` | Relevant CI green; verify post-merge checks, then perform test-environment QA. |
| `stage` | Production copy for final sign-off | PR from `test` | Test QA complete; staging configuration, deployment, and migration rehearsal confirmed. |
| `main` | Production. Every commit here is live or about to be. | PR from `stage`, or from `hotfix/*` | Staging approved; production configuration and migration readiness confirmed. Tag the release. |
...
- **Start each new feature from a clean, freshly pulled `dev`:** fetch `origin`, switch to `dev`, run `git pull --ff-only origin dev`, then create a new `feat/*` branch. Do not stack work on a previously merged feature branch. Keep a long-running branch current with `dev`.
- **Wait for relevant CI before merging each PR.** Do not bypass a failing check. After every merge, verify the remote target contains the change and its post-merge checks pass. Do not push feature code directly to a long-lived branch.
...
- **Stop when an environment is not ready.** Confirm its migrations, configuration, deployment, and required QA before a stage or production promotion; report the blocker instead of skipping a stage. The current environment status is in `infra/README.md`.
...
- Docs are updated when behaviour or a contract changes: this file, the part's `AGENTS.md` or README, and `docs/design/qa/design-qa.md` for design work.
...
- Add focused unit tests for nontrivial filtering, validation, and business rules. Run typecheck, import-boundary check, tests, and build before opening a PR. Check desktop and phone layouts in the browser.
...
- **Render hosting plan:** a Static Site for the frontend and a Web Service for the API, described in `infra/render.yaml`. Check `infra/README.md` for actual environment readiness before promotion.
