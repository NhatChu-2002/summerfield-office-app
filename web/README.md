...
The calendar module is lazy-loaded and uses FullCalendar 6 with its RRule integration rather than custom recurrence expansion. Run its focused date/validation tests with Node 22.18+ or Node 24: `node --test src/vy/calendar-model.test.mjs`. The next UI conversion is **Department folders**, followed by **How HQ works**.
