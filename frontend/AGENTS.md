...
This is the React single-page app that staff use. It builds to static files in `dist/`. It talks to Supabase with the signed-in user's session, and later it will talk to `backend/api`. Read `../AGENTS.md` for repository-wide rules.
...
│   ├── ownership/               Who to ask and Decision chart
│   ├── projects/                Projects dashboard/detail, preview records, model + tests
│   ├── watch/                   Market watch
...
└── shared/                      no product knowledge: usable by any feature
    ...
    ├── ui/                      shared controls (select, date, time), notices, toast, icons
...
The sign-in screen offers **Preview the HQ design** in development builds only. Signed-in admins get **Design preview** in the workspace. Preview uses `previewAccess` in `app/PreviewApp.tsx` and must never read or write company data. Calendar, ownership, watch, and project preview edits live in memory only. Signed-in screens must clearly mark unconnected services and never show sample records as company data.
...
- **Play:** `components/SunnyPlayMenu.tsx` is Vy's "Play with Sunny" menu. Performances (boba, dragonfly flight, dance, hug, nap) in `SunnyPet.tsx` run through `startPerformance()` and `later()`, so only one runs at a time, and `say()` cancels whatever is still running. Keep new moves starting and ending at the rest pose, and speak after the move.
