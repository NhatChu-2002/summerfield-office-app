...
**Projects** at `#/projects` and `#/project/:id` has a dashboard, filters, progress cards, a ticket desk, and project Tasks, Chat, Files, and Details views. Design preview uses sample projects and memory-only project, task, ticket, and message edits. These clear on reload. Signed-in mode shows real HQ task totals where available but no invented shared projects or ticket queue; project storage, chat, files, and Asana synchronization are not connected.
...
**Team reports** at `#/reports/:type/:period` and `#/report/:department/:type/:period` connects signed-in weekly and monthly reports to the existing `team_reports` RPCs. Weeks run Sunday through Saturday in the Los Angeles business timezone; months use calendar bounds. Store Manager reports are per accessible store. Version-one native payloads can be edited and saved with optimistic revision checks; leads and admins can submit, and only admins can reopen. Version-two inventory-form payloads are displayed read-only until that form is converted; unknown versions are never overwritten. The metric catalog snapshot in `src/features/reports/metrics-v1.json` comes from `../../inventory-app/data/team_report_templates.py` via `scripts/sync-report-metrics.py` and should be refreshed only after reviewing template changes. The index fetches each visible period identity because the current summaries RPC has no period filter. Design preview remains a separate monthly-only, memory-only editor with sample records, presentation, and preview CSV. AI summaries, report-to-task conversion, PDF output, and vendor-pricing reports are not connected in HQ.
...
Connected workflows now include HQ tasks, team updates, and version-one weekly/monthly Team Reports. The other prototype sections are UI-first previews until their shared services are integrated; this React app does not yet replace the full inventory report form, tickets, inventory, calendar, or department tools.
