...
export function DepartmentLayout({ department, role, taskContent, updateContent, onNewTask, writable, dataReady }: {
  ...
  return <>
    ...
    <div className="vy-department-columns"><div className="vy-stack"><Panel title="Coming up" action={<label className="vy-checkbox" title="Calendar visibility is available after the calendar migration"><input type="checkbox" checked disabled /> Show on my calendar</label>}><p className="vy-empty">Nothing on the {department.name} calendar in the next 90 days.</p></Panel>
      ...
      <Panel title="Locations we look after" action={<a className="vy-panel-link" href="#/locations">All locations <ArrowRight size={15} /></a>}><p className="vy-empty">{preview ? 'Open sample location files in the Design preview.' : 'Shared location records are not connected yet.'}</p></Panel>
      ...
      <Panel title="Shared records" action={<a className="vy-panel-link" href="#/catalog">Equipment catalog <ArrowRight size={15} /></a>}><p className="vy-empty">{preview ? 'The sample catalog is available to inspect in the Design preview.' : 'The shared equipment catalog is not connected yet.'}</p></Panel>
