...
import { compareTasks, TaskList, type HqTask, type PeopleNames } from '@/features/tasks'
import { UpdateComposer, UpdateList, type HqUpdate } from '@/features/updates'
import { DepartmentLayout } from '../components/DepartmentLayout'

export function DepartmentPage({ access, department, tasks, updates, names, busyTaskId, onToggle, onOpenTask, onNewTask, onPosted, dataReady }: {
  access: Access; department: Department; tasks: HqTask[]; updates: HqUpdate[]; names: PeopleNames;
  busyTaskId: string | null; onToggle: (task: HqTask) => void; onOpenTask: (task: HqTask) => void;
  ...
  return <DepartmentLayout
    ...
    taskContent={<TaskList tasks={departmentTasks} access={access} onToggle={onToggle} busyId={busyTaskId} empty="No tasks here yet." onOpen={onOpenTask} names={names} />}
