...
export default function ReportLibraryPage({ access, view }: { access: Access; view: View }) {
  ...
  return <>
    ...
    <div className="vy-report-library-filters">
      <label className="vy-report-library-search"><Search size={16} aria-hidden="true" /><span className="sr-only">Search report summaries</span><input type="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search summaries" maxLength={120} /></label>
